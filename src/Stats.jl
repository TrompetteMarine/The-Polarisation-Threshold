...
using Random, Statistics, LinearAlgebra, StatsBase
...
function estimate_Vstar(p::Params; N::Int=20_000, T::Float64=400.0, dt::Float64=0.01,
    ...
    # Initialize simulation with correct RNG
    rng = Random.MersenneTwister(seed)
    u = zeros(N)
    
    # Time evolution with proper index handling
    ...
    return var(u[burn_in_idx+1:end])
end

# Estimate g (baseline odd-mode decay) by injecting a small odd perturbation and measuring decay rate at κ=0
...
function sweep_kappa(p::Params, κgrid::Vector{Float64};
    ...
    for (k, κ) in enumerate(κgrid)
        ...
        amps[k] = abs(mean(u_valid))
        vars[k] = mean(abs2, u_valid)
    end
...
function critical_kappa(p::Params; g::Union{Nothing,Float64}=nothing, Vstar::Union{Nothing,Float64}=nothing,
                        N::Int=20_000, T::Float64=400.0, dt::Float64=0.01, burn_in::Float64=100.0, seed::Int=0)
...
function pitchfork_fit(res::KappaSweepResult; ε::Float64=1e-3, κmin::Union{Nothing,Float64}=nothing, κmax::Union{Nothing,Float64}=nothing)
    κ = res.κ
    A = res.amp
    mask = (A .> ε)
    if κmin !== nothing
