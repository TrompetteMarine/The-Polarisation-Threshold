...
function main()
    ...
    @add_arg_table s begin
        ...
        "--seed"; arg_type=Int; default=0; help="Random seed (0=none)"
        "--kappa"; arg_type=Float64; default=0.0; help="Coupling κ"
        "--mode"; arg_type=String; default="vstar"; help="vstar | sweep"
    end
    args = parse_args(s)

    p = Params(λ=args["lambda"], σ=args["sigma"], Θ=args["theta"], c0=args["c0"], hazard=StepHazard(args["nu0"]))

    if args["mode"] == "vstar"
        Vstar = estimate_Vstar(p; N=args["N"], T=args["T"], dt=args["dt"], burn_in=args["burnin"], seed=args["seed"])
    ...
    elseif args["mode"] == "sweep"
        ...
        res = BeliefSim.Stats.sweep_kappa(p, κgrid; N=args["N"], T=args["T"], dt=args["dt"], burn_in=args["burnin"], seed=args["seed"])
        println("# kappa\tamp\tvariance")
        for (κ, a, V) in zip(res.κ, res.amp, res.V)
            println(string(κ, '\t', a, '\t', V))
        end
    else
        error("Unknown mode")
    end
